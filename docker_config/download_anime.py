...
while p.poll() is None:
    for source, output in iter(q.get, None):
        ...
        if "nach dem du suchen willst" in output and GOT_FIRST_EXIT_MESSAGE == False:
            time.sleep(3)
            ARG1_INPUT=sys.argv[1].replace("HENTAI_", "")
            print("Sending input: " + ARG1_INPUT)
            p.stdin.write((ARG1_INPUT + "\n").encode())
            p.stdin.flush()
            p.stdout.flush()
            sys.stdout.flush()
        ...
        if ("[1] Name:" in output and ("Episoden: 1337" in output or "1337/1337" in output or "movie," in output)) or 'I_AM_A_MOVIE!' in output:
        ...
        if "Wieviel Episoden hast du bereits runtergeladen" in output and "Fehlerhafte Eingabe" in output:
            if SKIP_EPISODES > 0:
                # skip X episodes
                print("Skipping " + str(SKIP_EPISODES) + " episodes and only downloaind next ones.")
                print("Sending input: " + str(SKIP_EPISODES))
                p.stdin.write((str(SKIP_EPISODES) + "\n").encode())
                p.stdin.flush()
                p.stdout.flush()
                sys.stdout.flush()
            else:
        ...
        if "Wieviel Episoden hast du bereits runtergeladen" in output and "Fehlerhafte Eingabe" not in output:
            ...
            ARG1_INPUT_BACKUP=sys.argv[1]
            ARG1_INPUT=sys.argv[1]
            
            if "http" in ARG1_INPUT and "/media/" in ARG1_INPUT:
                ARG1_INPUT = ARG1_INPUT.split("/media/")[1]
                if "HENTAI_" in ARG1_INPUT_BACKUP:
                    ARG1_INPUT = "HENTAI_" + ARG1_INPUT
            
            print("Sending input: " + (ARG1_INPUT + " " + lang_for_package_name + " " + RESOLUTION + " " + SEARCH_RESULT_TYPE))
            ...
            p.stdin.write((ARG1_INPUT + " " + lang_for_package_name + " " + RESOLUTION + " " + SEARCH_RESULT_TYPE + "\n").encode())
            p.stdin.flush()
            p.stdout.flush()
            sys.stdout.flush()
