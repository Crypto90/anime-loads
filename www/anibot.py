...
def addAnime():
    ...
    while(exit == False):
        ...
        if("https://www.anime-loads.org/media/" in aniquery):
            ...
            isMovieCheck = anime.getType().lower()
            print("================isMovieCheck================")
            print(isMovieCheck)
            print("============================================")
            if 'movie' in isMovieCheck or 'film' in isMovieCheck:
                print("I_AM_A_MOVIE!")
            print("\n")
            ...
            animedata = {
                ...
                "missing": [],
                "releaseID": relchoice,
                "releaseUID": release.getReleaseUID(),
        ...
        if(anidata != ""):
            for idx, animeentry in enumerate(anidata):
                ...
                try:
                    ...
                    print("Failed to get Anime '" + name + "' (" + url + "), skipping...")
                    with open(botfile, "r") as f:
                        ...
                        for reloaded_idx, reloaded_animeentry in enumerate(reloaded_anidata):
                            if (reloaded_animeentry['name'] == name):
                                reloaded_animeentry['status'] = "Release fehlerhaft. Anime oder Release fehlt."
                ...
                releaseUID = ""
                if 'releaseUID' in animeentry:
                    releaseUID = animeentry['releaseUID']
                            ...
                            with open(botfile, "r") as f:
                                ...
                                for reloaded_idx, reloaded_animeentry in enumerate(reloaded_anidata):
                                    if (reloaded_animeentry['name'] == name):
                                        ...
                                        if releaseUID == "":
                                            reloaded_animeentry['releaseUID'] = release.getReleaseUID()
                                        reloaded_animeentry['maxEpisodes'] = anime.getMaxEpisodes()
                                        # Update the reloaded data with the modified animeentry
                                        reloaded_anidata[reloaded_idx] = reloaded_animeentry
                                        reloaded_data['anime'] = reloaded_anidata
                ...
                if releaseUID != "" and releaseUID != release.getReleaseUID():
                    print("[ERROR]" + name + " Release fehlerhaft!!!!! Release UID stimmt nicht mehr überein!")
                    with open(botfile, "r") as f:
                        reloaded_data = json.load(f)
                        reloaded_anidata = reloaded_data.get('anime', [])
                        ...
                        for reloaded_idx, reloaded_animeentry in enumerate(reloaded_anidata):
                            if (reloaded_animeentry['name'] == name):
                                reloaded_animeentry['status'] = "Release fehlerhaft. ReleaseUID falsch."
                                reloaded_animeentry['maxEpisodes'] = anime.getMaxEpisodes()
                                # Update the reloaded data with the modified animeentry
                                reloaded_anidata[reloaded_idx] = reloaded_animeentry
                                reloaded_data['anime'] = reloaded_anidata
                    ...
                    for i in range(episodes + 1, curEpisodes + 1):
                        ...
                        if(dl_ret == True):
                            ...
                            log("[ERROR] Fehler beim runterladen von Episode " + str(i) + " von " + name + ", breche den aktuellen Durchlauf für diesen Anime ab und versuche es beim nächsten Mal erneut.", pb)
                            break
                #elif int(episodes) == curEpisodes:
                else:
                ...
                #else:
                #    print("[ERROR]" + name + " Release fehlerhaft!!!!!!!!!!!")
                #    with open(botfile, "r") as f:
                #        reloaded_data = json.load(f)
                #        reloaded_anidata = reloaded_data.get('anime', [])
                #        
                #        for reloaded_idx, reloaded_animeentry in enumerate(reloaded_anidata):
                #            if (reloaded_animeentry['name'] == name):
                #                reloaded_animeentry['status'] = "Release fehlerhaft"
                #                reloaded_animeentry['maxEpisodes'] = anime.getMaxEpisodes()
                #                # Update the reloaded data with the modified animeentry
                #                reloaded_anidata[reloaded_idx] = reloaded_animeentry
                #                reloaded_data['anime'] = reloaded_anidata
