...
class animeloads:
    ...
    def _create_browser(self, browser_type, browserloc=""):
        import time
        import selenium.common.exceptions
        for attempt in range(3):
            try:
                if browser_type == animeloads.CHROME:
                    options = selenium.webdriver.chrome.options.Options()
                    options.add_argument("--headless")
                    options.add_argument("--no-sandbox") # Verhindert Crashes im Container
                    options.add_argument("--disable-dev-shm-usage") # Verhindert Out-of-Memory Crashes
                    if browserloc != "":
                    ...
                    try:
                        ...
                        return webdriver.Chrome(service_log_path=os.devnull, options=options)
                elif browser_type == animeloads.FIREFOX:
                    ...
                    try:
                        ...
                        return webdriver.Firefox(service_log_path=os.devnull, options=options)
                else:
                    raise ALInvalidBrowserException("Nicht unterstützter Browser")
            except selenium.common.exceptions.WebDriverException as e:
                if attempt == 2:
                    raise e
                print(f"Browser Start fehlgeschlagen (Versuch {attempt+1}/3), probiere erneut...")
                time.sleep(2)
        return None
    ...
    def __init__(self, user="", pw="", browser="", browserloc=""):
        ...
        driver = self._create_browser(browser, browserloc)
...
#                self.session.get(j.split("\"")[0])
        ...
        try:
            ...
            # Define XPath expression to select button elements within div elements with id starting with 'download_'
            #xpath_expr = '//button[contains(@data-target, "#report")]/@data-release'
            xpath_expr = "//div[@id='downloads']//button[contains(@class, 'notify')]/@data-md5"
            # Find all matching elements
            melden_buttons = rel_dom.xpath(xpath_expr)
            
            #print("=================================abonieren_buttons_md5=================================")
            #print(melden_buttons)
            #print("====================================================================================")
            #print("numReleases: " + str(numReleases))
            #print("====================================================================================")
            ...
            for relIndex in range(0, numReleases):
                ...
                releaseUID = melden_buttons[relIndex]
                episodes = 0
    ...
    #                print("Episodenumber: " + str(epnum))
                ...
                tmprel = release(self.url, relID, group, res, dubs, subs, epnum, videoformat, size, password, anmerkung, detailPage.text, self.session, releaseUID)
    ...
    def downloadEpisode(self, episode, release, hoster, browser, browserlocation="", jdhost="", myjd_user="", myjd_pw="", myjd_device="",jd_deprecated=False,jd_deprecatedport="3128", pkgName="", destinationFolder=""):
        ...
        # rm animeloads.py; wget https://anime.shieldserver.de/animeloads.py
        # rm anibot.py; wget https://anime.shieldserver.de/anibot.py
...
#        print("\n\n\n\ndl_Other cookies: " + str(r.cookies))
        ...
        print("Starte browser für download link parsing...")
        driver = self.animeloads._create_browser(browser, browserlocation)
        ...
        # noadblock-Fix: Hardcodierte Cookies, damit der JS-Check der Seite ruhig bleibt
        driver.add_cookie({'name': 'adsblocked', 'value': '0', 'domain': '.anime-loads.org', 'path': '/'})
        driver.add_cookie({'name': 'hideads', 'value': '1', 'domain': '.anime-loads.org', 'path': '/'})

        print("Open anime url: " + self.url + " ...")
        driver.get(self.url)
        time.sleep(1.5) # Etwas Warten, damit Cookies greifen und Scripte fertig laden

        print("Parsing download link ...")
...
#            raise ALCaptchaException("Captcha is needed to get download links")
        ...
        if(code != "success"):
            driver.quit()
            # FIX: raise anstelle von return nutzen, sonst geht die Routine davon aus, dass alles geklappt hat!
            raise ALUnknownException("Ein unbekannter Fehler (z.B. noadblock) beim Lesen der Hosterlinks ist aufgetreten oder das Captcha konnte nach 5 Versuchen nicht gelöst werden.")
        ...
        # Fallback, falls der gewählte Hoster offline / für dieses Release nicht vorhanden ist:
        if not cnldata:
            print("Bevorzugter Hoster nicht gefunden, suche nach alternativen Hostern...")
            if hoster == animeloads.DDOWNLOAD and content_rapid:
                cnldata = content_rapid
            elif hoster == animeloads.RAPIDGATOR and content_ddl:
                cnldata = content_ddl
            else:
                raise ALLinkExtractionException("Kein passender Hoster (DDownload/Rapidgator) in der Antwort gefunden.")
        ...
        # Key-Obfuscation (Tausch 15. und 16. Zeichen) sicher wiederherstellen
        if k and len(k) >= 17:
            k_list = list(k)
            tmp = k_list[15]
            k_list[15] = k_list[16]
            ...
            raise ALLinkExtractionException("Linkdaten/AES-Key konnten nicht gelesen werden oder Key ist ungueltig.")
        ...
        if(jdhost != "" and myjd_user == "" and not jd_deprecated):
            ...
            try:
                ...
                if not links:
                    raise ALUnknownException("Keine Links nach Entschluesselung uebrig. AES Key falsch?")
                ...
                print("Failed to add Link to MyJD:", str(e))
                return False
            ...
            try:
                ...
                print("Failed to add Link to JD (deprecated):", str(e))
                return False
            ...
            try:
                ...
                print("Failed to decode links:", str(e))
                return False
...
class release:
    ...
    def __init__(self, url, rid, group, res, dubs, subs, episodeCount, videoformat, size, password, anmerkung="", sitesource="", session="", releaseUID=""):   #TODO sortieren
        self.url = url
        self.rid = rid
        self.group = group
        self.res = res
    ...
    def getReleaseUID(self):
        return self.releaseUID
    ...
    def tostring(self):
        return "Release ID: " + str(self.rid) + ", Group: " + self.group + ", Resolution: " + self.res + ", Videocodec: " + self.videocodec + ", Audiocodec: " + self.audiocodec + ", Dubs: " + str(self.dubs) + \
            ", Subs: " + str(self.subs) + ", Format: " + self.videoformat + ", Size: " + str(self.size) + "MB, Password: " + self.password + ", Release Anmerkungen: " + self.anmerkung
